...
def create_dataset(df):
    ...
    for _, row in df.iterrows():
        ...
        dataset.append((tokens_feat,image_data, label))
    return dataset


train_dataset = create_dataset(train_df)
print(len(train_df))
torch.save(train_dataset, 'datasets/trainset_baseline_transformer_cnn.pt')

test_df = pd.read_csv('datasets/testset.csv')
test_dataset = create_dataset(test_df)
torch.save(test_dataset, 'datasets/testset_baseline_transformer_cnn.pt')
